...
class TurnLayout:
    ...
    COMMAND_HELP = "Choose: a (first option), b (second option), g (regenerate), r (restart), q (quit)"

    def __init__(self, console: Console) -> None:
        self.console = console

    def turn_status_label(self, turn_number: int, provider_summary: str, tokens_used: int) -> str:
        return f"turn {turn_number} | {provider_summary} | tokens used: {tokens_used}"

    def art_width(self) -> int:
        return max(40, (self.console.width or 100) - 4)

    def art_height(self, story: str, option_a: str, option_b: str) -> int:
        total_lines = self.console.height or 24
        non_art_lines = self.estimated_non_art_line_count(story, option_a, option_b)
    ...
    def _estimated_non_art_line_count_at_width(
        ...
        *,
        ...
        command_lines = 1 + self._command_lines(width=width)

        return outer_lines + story_panel_lines + choices_lines + command_lines

    def _command_lines(self, width: int | None = None) -> int:
